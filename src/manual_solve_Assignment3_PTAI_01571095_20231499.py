...
# Git Link(s):  "https://github.com/paddy-garrett/ARC" , "https://github.com/tapanauti/ARC"
...
As python uses referencing and does not actually copy the values,
so to use the passed x for solve functions here everywhere we have made a deepcopy of x to solve the problem.

Solving matrix-related problems in the assignment clearly involves extensive iteration throughmulti-dimensional arrays
and for the most part we accomplished this through pure python by the use of for-loops. 
However, Numpy provided us with several useful tools to accomplish our tasks, in one solve function we made use of the scipy ndimage modules.

The reshape function proved very useful method for returning “slices” of multiple lines of the matrix in their original shape. 

The transpose function in numpy provides an efficient way of iterating downwards into a matrix by switching rows and columns of the grid.

Numpy.zeros function is used to create a multi-dimensional array of zeros (which had useful applications for us in solving ARC tasks). 

Numpy.fill_diagonal is used to fill the diagonal of the matrix with desired values.

ndimage.label function is used to return the labeled non-connected blocks in a grid, so this helped us to retrieve the number of non-connected shapes in grid.
...
All test and training cases are solved correctly.
"""
...
"""8a004b2b: The task grid here is a around 19 x 18 grid, 
 the solution grid is depending upon the values in input grid i.e the subgrid consisting of all four corners in yellow and matching the pattern from below figure in grid .

Here I was able to print the frame of yellow border but didn't complete it to match the figure below and get the final solution.
 
None of the training and testing case is solved correctly 
